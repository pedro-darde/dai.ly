...
import TaskRepository, {
  TaskDatabase,
  TaskWithNoteFlag,
} from "../../domain/repository/TaskRepository";
...
export default class TaskRepositoryDatabase implements TaskRepository {
  ...
  async edit(id: number, task: Task): Promise<void> {
    await this.connection.query(
      "UPDATE phd.tasks SET title = $1,about = $2,expected_time = $3,start_at = $4, ended_at = $5, time_spent = $6,status = $7, expected_date = $8 where id = $9",
      [
        task.title,
        task.about,
        task.expectedTime,
  ...
  async remove(id: number, hasNotes: boolean): Promise<void> {
    if (hasNotes) {
      await this.connection.query(
        "DELETE FROM phd.task_note WHERE id_task = $1",
        [id]
      );
    }
  ...
  async markAsDone(id: number, timeSpent: number): Promise<void> {
    await this.connection.query(
      "UPDATE phd.tasks SET time_spent = $1, status = $2, ended_at = $3 WHERE id = $4",
      [timeSpent, TaskStatus.VALIDATED, new Date(), id]
    );
  }

  async getValidatedAndRejected(): Promise<TaskWithNoteFlag[]> {
    const tasksData = await this.connection.query<TaskDatabase[]>(
      "SELECT task.*, array_agg(tn.id_note) as notes FROM phd.tasks task LEFT JOIN phd.task_note tn ON tn.id_task = task.id WHERE task.status in($1, $2) GROUP BY task.id ORDER BY task.id ",
      [TaskStatus.VALIDATED, TaskStatus.REJECTED]
    );
    const tasks: TaskWithNoteFlag[] = [];
