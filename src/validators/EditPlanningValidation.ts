...
export default class EditPlanningValidation implements Validation {
  ...
  private async validateMonths(monthsToAdd: ToAdd[]): Promise<void | Error> {
  ...
  private validateMonthsAndItems(months: ToAdd[] | ToUpdate[]) {
    const validationMonth = new RequiredArrayFieldsValidation(
      "months",
      this.MONTH_REQUIRED_FIELDS
    );
    const monthError = validationMonth.validate({
      months: this.createMonthToValidate(months),
    });
  ...
  private validateItems(months: any) {
    ...
    const itemValidation = new RequiredArrayFieldsValidation(
      "items",
      this.MONTH_ITEM_REQUIRED_FIELDS
    );
    const itemError = itemValidation.validate({
      items: this.createItemToValidate(monthItems),
    });
    if (itemError) return itemError;
  }
